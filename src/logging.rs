...
pub fn log_query_points_as_maps(
    writer: &mut SummaryWriter,
    ...
    for ray_points in query_points {
        for [world_x, world_y, world_z] in ray_points {
            println!("xyz  world {:?}", [world_x, world_y, world_z]);
            ...
            println!("xyz usize {:?}", [x, y, z]);
            backbuffer_yx[(y * 100 + x).min(10000 - 1)] =
                prediction_array_as_u32(&[1., 1., 1., 1.]);
            backbuffer_zx[(z * 100 + x).min(10000 - 1)] =
                prediction_array_as_u32(&[1., 1., 1., 1.]);
            backbuffer_yz[(y * 100 + z).min(10000 - 1)] =
                prediction_array_as_u32(&[1., 1., 1., 1.]);
        }
    }
